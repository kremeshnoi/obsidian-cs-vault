...
Methods are similar to functions: We declare them with the `fn` keyword and a name, they can have parameters and a return value, and they contain some code that’s run when the method is called from somewhere else. Unlike functions, methods are defined within the context of a struct (or an enum or a trait object, which we cover in [[4 Enums and Pattern Matching|Chapter 6]] and [[16.2 Using Trait Objects to Abstract over Shared Behavior|Chapter 18]], respectively), and their first parameter is always `self`, which represents the instance of the struct the method is being called on.
...
[Listing 5-13](https://doc.rust-lang.org/book/#listing-5-13): Defining an `area` method on the `Rectangle` struct
...
Often, but not always, when we give a method the same name as a field we want it to only return the value in the field and do nothing else. Methods like this are called *getters*, and Rust does not implement them automatically for struct fields as some other languages do. Getters are useful because you can make the field private but the method public and thus enable read-only access to that field as part of the type’s public API. We will discuss what public and private are and how to designate a field or method as public or private in [[5.3 Paths for Referring to an Item in the Module Tree|Chapter 7]].
...
[Listing 5-14](https://doc.rust-lang.org/book/#listing-5-14): Using the as-yet-unwritten `can_hold` method
...
[Listing 5-15](https://doc.rust-lang.org/book/#listing-5-15): Implementing the `can_hold` method on `Rectangle` that takes another `Rectangle` instance as a parameter
...
To call this associated function, we use the `::` syntax with the struct name;`let sq = Rectangle::square(3);` is an example. This function is namespaced by the struct: The `::` syntax is used for both associated functions and namespaces created by modules. We’ll discuss modules in [[5.2 Control Scope and Privacy with Modules|Chapter 7]].
...
[Listing 5-16](https://doc.rust-lang.org/book/#listing-5-16): Rewriting Listing 5-15 using multiple `impl` blocks
