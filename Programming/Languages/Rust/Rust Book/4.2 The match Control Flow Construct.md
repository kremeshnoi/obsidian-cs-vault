...
Rust has an extremely powerful control flow construct called `match` that allows you to compare a value against a series of patterns and then execute code based on which pattern matches. Patterns can be made up of literal values, variable names, wildcards, and many other things; [[17 Patterns and Matching|Chapter 19]] covers all the different kinds of patterns and what they do. The power of `match` comes from the expressiveness of the patterns and the fact that the compiler confirms that all possible cases are handled.
...
[Listing 6-3](https://doc.rust-lang.org/book/#listing-6-3): An enum and a `match` expression that has the variants of the enum as its patterns
...
[Listing 6-4](https://doc.rust-lang.org/book/#listing-6-4): A `Coin` enum in which the `Quarter` variant also holds a `UsState` value
...
[Listing 6-5](https://doc.rust-lang.org/book/#listing-6-5): A function that uses a `match` expression on an `Option<i32>`
...
Finally, we’ll change the rules of the game one more time so that nothing else happens on your turn if you roll anything other than a 3 or a 7. We can express that by using the unit value (the empty tuple type we mentioned in [[1.2 Data Types|“The Tuple Type”]] section) as the code that goes with the `_` arm:
...
There’s more about patterns and matching that we’ll cover in [[17 Patterns and Matching|Chapter 19]]. For now, we’re going to move on to the `if let` syntax, which can be useful in situations where the `match` expression is a bit wordy.