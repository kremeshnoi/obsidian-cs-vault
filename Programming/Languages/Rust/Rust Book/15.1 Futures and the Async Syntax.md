...
[Listing 17-1](https://doc.rust-lang.org/book/#listing-17-1): Defining an async function to get the title element from an HTML page
...
[Listing 17-2](https://doc.rust-lang.org/book/#listing-17-2): Chaining with the `await` keyword
...
- It uses the `impl Trait` syntax we discussed back in Chapter 10 in the [[8.2 Defining Shared Behavior with Traits|“Traits as Parameters”]] section.
...
[Listing 17-3](https://doc.rust-lang.org/book/#listing-17-3): Calling the `page_title` function from `main` with a user-supplied argument

We follow the same pattern we used to get command line arguments in the [[10.1 Accepting Command Line Arguments|“Accepting Command Line Arguments”]] section in Chapter 12. Then we pass the URL argument to `page_title` and await the result. Because the value produced by the future is an `Option<String>`, we use a `match` expression to print different messages to account for whether the page had a `<title>`.
...
[Listing 17-4](https://doc.rust-lang.org/book/#listing-17-4): Awaiting an async block with `trpl::block_on`
...
[Listing 17-5](https://doc.rust-lang.org/book/#listing-17-5): Calling `page_title` for two URLs to see which returns first
