...
[Listing 15-6](https://doc.rust-lang.org/book/#listing-15-6): Using the dereference operator to follow a reference to an `i32` value
...
[Listing 15-7](https://doc.rust-lang.org/book/#listing-15-7): Using the dereference operator on a `Box<i32>`
...
[Listing 15-8](https://doc.rust-lang.org/book/#listing-15-8): Defining a `MyBox<T>` type
...
[Listing 15-9](https://doc.rust-lang.org/book/#listing-15-9): Attempting to use `MyBox<T>` in the same way we used references and `Box<T>`
...
[Listing 15-10](https://doc.rust-lang.org/book/#listing-15-10): Implementing `Deref` on `MyBox<T>`
...
We fill in the body of the `deref` method with `&self.0` so that `deref` returns a reference to the value we want to access with the `*` operator; recall from [[3.1 Defining and Instantiating Structs|“Creating Different Types with Tuple Structs”]] in Chapter 5 that `.0` accesses the first value in a tuple struct. The `main` function in Listing 15-9 that calls `*` on the `MyBox<T>` value now compiles, and the assertions pass!
...
[Listing 15-11](https://doc.rust-lang.org/book/#listing-15-11): A `hello` function that has the parameter `name` of type `&str`
...
[Listing 15-12](https://doc.rust-lang.org/book/#listing-15-12): Calling `hello` with a reference to a `MyBox<String>` value, which works because of deref coercion
...
[Listing 15-13](https://doc.rust-lang.org/book/#listing-15-13): The code we would have to write if Rust didn’t have deref coercion
