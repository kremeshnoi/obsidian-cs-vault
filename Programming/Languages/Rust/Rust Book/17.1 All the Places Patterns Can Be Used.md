...
The particular pattern `_` will match anything, but it never binds to a variable, so it’s often used in the last match arm. The `_` pattern can be useful when you want to ignore any value not specified, for example. We’ll cover the `_` pattern in more detail in [[17.3 Pattern Syntax|“Ignoring Values in a Pattern”]] later in this chapter.
...
[Listing 19-1](https://doc.rust-lang.org/book/#listing-19-1): Using a pattern to destructure a tuple and create three variables at once
...
[Listing 19-2](https://doc.rust-lang.org/book/#listing-19-2): Incorrectly constructing a pattern whose variables don’t match the number of elements in the tuple
...
To fix the error, we could ignore one or more of the values in the tuple using `_` or `..`, as you’ll see in the [[17.3 Pattern Syntax|“Ignoring Values in a Pattern”]] section. If the problem is that we have too many variables in the pattern, the solution is to make the types match by removing variables so that the number of variables equals the number of elements in the tuple.
...
[Listing 19-3](https://doc.rust-lang.org/book/#listing-19-3): Mixing `if let`, `else if`, `else if let`, and `else`
...
[Listing 19-4](https://doc.rust-lang.org/book/#listing-19-4): Using a `while let` loop to print values for as long as `rx.recv()` returns `Ok`
...
[Listing 19-5](https://doc.rust-lang.org/book/#listing-19-5): Using a pattern in a `for` loop to destructure a tuple
...
[Listing 19-6](https://doc.rust-lang.org/book/#listing-19-6): A function signature using patterns in the parameters
...
[Listing 19-7](https://doc.rust-lang.org/book/#listing-19-7): A function with parameters that destructure a tuple
