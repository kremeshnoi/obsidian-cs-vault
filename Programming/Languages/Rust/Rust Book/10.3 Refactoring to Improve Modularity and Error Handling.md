...
[Listing 12-5](https://doc.rust-lang.org/book/#listing-12-5): Extracting a `parse_config` function from `main`
...
[Listing 12-6](https://doc.rust-lang.org/book/#listing-12-6): Refactoring `parse_config` to return an instance of a `Config` struct
...
[Listing 12-7](https://doc.rust-lang.org/book/#listing-12-7): Changing `parse_config` into `Config::new`
...
[Listing 12-8](https://doc.rust-lang.org/book/#listing-12-8): Adding a check for the number of arguments

This code is similar to [[7.3 To panic! or Not to panic!|the `Guess::new` function we wrote in Listing 9-13]], where we called `panic!` when the `value` argument was out of the range of valid values. Instead of checking for a range of values here, we’re checking that the length of `args` is at least `3` and the rest of the function can operate under the assumption that this condition has been met. If `args` has fewer than three items, this condition will be `true`, and we call the `panic!` macro to end the program immediately.
...
This output is better: We now have a reasonable error message. However, we also have extraneous information we don’t want to give to our users. Perhaps the technique we used in Listing 9-13 isn’t the best one to use here: A call to `panic!` is more appropriate for a programming problem than a usage problem,[[7.3 To panic! or Not to panic!|as discussed in Chapter 9]]. Instead, we’ll use the other technique you learned about in Chapter 9— [[7.2 Recoverable Errors with Result|returning a `Result`]] that indicates either success or an error.
...
[Listing 12-9](https://doc.rust-lang.org/book/#listing-12-9): Returning a `Result` from `Config::build`
...
Now that we’ve finished refactoring the configuration parsing, let’s turn to the program’s logic. As we stated in [“Separating Concerns in Binary Projects”](https://doc.rust-lang.org/book/#separation-of-concerns-for-binary-projects), we’ll extract a function named `run` that will hold all the logic currently in the `main` function that isn’t involved with setting up configuration or handling errors. When we’re done, the `main` function will be concise and easy to verify by inspection, and we’ll be able to write tests for all the other logic.
...
[Listing 12-11](https://doc.rust-lang.org/book/#listing-12-11): Extracting a `run` function containing the rest of the program logic
...
[Listing 12-12](https://doc.rust-lang.org/book/#listing-12-12): Changing the `run` function to return `Result`
...
For the error type, we used the trait object `Box<dyn Error>` (and we brought `std::error::Error` into scope with a `use` statement at the top). We’ll cover trait objects in [[16 Object Oriented Programming Features|Chapter 18]]. For now, just know that `Box<dyn Error>` means the function will return a type that implements the `Error` trait, but we don’t have to specify what particular type the return value will be. This gives us flexibility to return error values that may be of different types in different error cases. The `dyn` keyword is short for *dynamic*.

Second, we’ve removed the call to `expect` in favor of the `?` operator, as we talked about in [[7.2 Recoverable Errors with Result|Chapter 9]]. Rather than `panic!` on an error, `?` will return the error value from the current function for the caller to handle.
...
[Listing 12-13](https://doc.rust-lang.org/book/#listing-12-13): Defining the `search` function in *src/lib.rs*
...
[Listing 12-14](https://doc.rust-lang.org/book/#listing-12-14): Using the `minigrep` library crate’s `search` function in *src/main.rs*
