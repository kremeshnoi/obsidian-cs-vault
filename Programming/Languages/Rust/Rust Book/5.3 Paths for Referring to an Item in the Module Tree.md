...
The `eat_at_restaurant` function is part of our library crate’s public API, so we mark it with the `pub` keyword. In the [[5.3 Paths for Referring to an Item in the Module Tree|“Exposing Paths with the `pub` Keyword”]] section, we’ll go into more detail about `pub`.
...
[Listing 7-3](https://doc.rust-lang.org/book/#listing-7-3): Calling the `add_to_waitlist` function using absolute and relative paths
...
[Listing 7-4](https://doc.rust-lang.org/book/#listing-7-4): Compiler errors from building the code in Listing 7-3
...
[Listing 7-5](https://doc.rust-lang.org/book/#listing-7-5): Declaring the `hosting` module as `pub` to use it from `eat_at_restaurant`
...
[Listing 7-6](https://doc.rust-lang.org/book/#listing-7-6): Compiler errors from building the code in Listing 7-5
...
[Listing 7-7](https://doc.rust-lang.org/book/#listing-7-7): Adding the `pub` keyword to `mod hosting` and `fn add_to_waitlist` lets us call the function from `eat_at_restaurant`.
...
In [[10 An IO Project Building a Command Line Program|Chapter 12]], we’ll demonstrate this organizational practice with a command line program that will contain both a binary crate and a library crate.
...
[Listing 7-8](https://doc.rust-lang.org/book/#listing-7-8): Calling a function using a relative path starting with `super`
...
[Listing 7-9](https://doc.rust-lang.org/book/#listing-7-9): A struct with some public fields and some private fields
...
[Listing 7-10](https://doc.rust-lang.org/book/#listing-7-10): Designating an enum as public makes all its variants public.
